type ('env, 'dag_data, 'pow) actions =
  ...
  ; extend_dag :
      ?pow:'pow -> ?sign:bool -> 'env Dag.node list -> 'dag_data -> 'env Dag.node
...
type ('env, 'dag_data) global_view =
  ...
  ; data : 'env Dag.node -> 'dag_data (** Read the protocol data attached to DAG node. *)
...
type ('env, 'dag_data, 'pow, 'state) protocol =
  ...
  ; dag_validity : ('env, 'dag_data) global_view -> 'env Dag.node -> bool
