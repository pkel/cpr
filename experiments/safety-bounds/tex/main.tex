...
\begin{algorithm}
  ...
  \While{true}{
    ...
    \ElseIf{$s = \texttt{Committed}$ \textbf{and} $a < d - c$}{
      ...
      Sample $x$ from geometric distribution with parameter $p = (1 - \alpha) e^{-\lambda\Delta}$\;
...
\begin{algorithm}
  ...
  \While{true}{
    ...
    \ElseIf{$s = \texttt{Committed}$ \textbf{and} $a < d - c$}{
      ...
      Sample $x$ from geometric distribution with parameter $p'$ from Obs.\,\ref{obs:ptick}\;
      \uIf(\tcc*[f]{safety violation feasible}){$x > d - a$}{
        \Return \texttt{Success}\;
      }
      \Else(\tcc*[f]{safety violation infeasible}){
...
\begin{algorithm}
  ...
  \While{true}{
    ...
    \Switch{e}{
      \uCase{\texttt{Proof-of-Work}}{
        ...
        \Else(\tcc*[f]{defender block}){
          \If(\tcc*[f]{Observation~\ref{obs:sync}}){$t > \tau$ \textbf{and} $a \leq d = d'$ \textbf{and} $\delta > \Delta$}{
            Insert \texttt{Sync} into $Q$ at time $t + \Delta$\;
          }
          $d \gets \max(d, d' + 1)$\tcc*{extends shortest chain}
      ...
      \Case(\tcc*[f]{Observation~\ref{obs:sync}}){\texttt{Sync}}{
        $f \gets \texttt{False}$\;
      }
    }
    \tcc{check termination}
