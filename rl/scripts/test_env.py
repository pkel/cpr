...
gamma = 0.9
defenders = 50
# model_names = ["2ys8h4ek", "3vnzkvsy"]
model_names = ["2x496teq"]
df = []
loaded_models = dict()
setups = list(itertools.product(reward_schemes, alphas, model_names, protos))
...
df = pd.DataFrame(df)
pd.to_pickle(df, f"rl/saved_models/model_evals_gamma_{gamma}.pkl")
